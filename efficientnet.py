...
def EfficientNet(width_coefficient,
    ...
    x = layers.Conv2D(round_filters(32, width_coefficient, depth_divisor), 3,
                      ...
                      use_bias=False,
                      kernel_initializer=CONV_KERNEL_INITIALIZER,
                      name=sianum + 'stem_conv')(x)
    x = layers.BatchNormalization(axis=bn_axis, name=sianum + 'stem_bn')(x)
    x = layers.Activation(activation, name=sianum + 'stem_activation')(x)
    ...
    for idx, block_args in enumerate(blocks_args):
        ...
        x = mb_conv_block(x, block_args,
                          activation=activation,
                          drop_rate=drop_rate,
                          prefix='{}_block{}a_'.format(sianum, idx + 1))
        block_num += 1
    ...
    x = layers.Conv2D(round_filters(1280, width_coefficient, depth_divisor), 1,
                      ...
                      use_bias=False,
                      kernel_initializer=CONV_KERNEL_INITIALIZER,
                      name=sianum + 'top_conv')(x)
    x = layers.BatchNormalization(axis=bn_axis, name=sianum + 'top_bn')(x)
    x = layers.Activation(activation, name=sianum + 'top_activation')(x)
    if include_top:
        ...
        if pooling == 'avg':
            x = layers.GlobalAveragePooling2D(name=sianum + 'avg_pool')(x)
        elif pooling == 'max':
            x = layers.GlobalMaxPooling2D(name=sianum + 'max_pool')(x)
    ...
    # Create model.
    modelnamelist = model_name.split('-')
    modelname = modelnamelist[0]
    model = models.Model(inputs, x, name=sianum + '-' + modelname)
    ...
    if weights == 'imagenet':
        ...
        # model.load_weights('/ABC2/0_lrh/mymymy/efficientnetModel/efficientnet-b6_weights_tf_dim_ordering_tf_kernels_autoaugment_notop.h5')
        model.load_weights('/media/node/Code/lrh/code/efficientnetModel/efficientnet-b7_weights_tf_dim_ordering_tf_kernels_autoaugment_notop.h5')
        # model.load_weights('/media/node/Code/Pycharm_Programs/ls_sal/efficientnetModel/efficientnet-l2_noisy-student_notop.h5')
...
def EfficientNetB0(
    ...
    return EfficientNet(
        ...
        pooling=pooling, classes=classes,sianum=sianum,
        **kwargs
    )

...
def preprocess_sammaps(paths, shape_r_sam, shape_c_sam):
    ims = np.zeros((len(paths), 1, shape_r_sam, shape_c_sam))

    for i, path in enumerate(paths):
        original_image = cv2.imread(path, 0)
        padded_image = padding(original_image, shape_r_sam, shape_c_sam, 1)
        ims[i] = padded_image
...
def preprocess_fixmaps(paths, shape_r, shape_c):
    ims = np.zeros((len(paths), shape_r, shape_c, 24))

    for i, path in enumerate(paths):
        act_map = scipy.io.loadmat(path)
        act_image = act_map['activate_mat']
        paddedact_map = padding(act_image, shape_r, shape_c, 24)
        ims[i] = paddedact_map
